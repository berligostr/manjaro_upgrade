...
postrunif () 
{
  ...
  if [[ -f $HOME/upgrade.pamac ]]; then echo -e "\n"; if cat $HOME/upgrade.pamac | grep "$1" > /dev/null ; then rm $HOME/upgrade.pamac; fi; fi
  if [[ -f $HOME/upgrade.pamac ]]; then echo -e "\n"; if cat $HOME/upgrade.pamac | grep "$2" > /dev/null ; then rm $HOME/upgrade.pamac; fi; fi
  if [[ -f $HOME/upgrade.yay ]]; then echo -e "\n"; if cat $HOME/upgrade.yay | grep "$3" > /dev/null ; then rm $HOME/upgrade.yay; fi; fi
  if [[ -f $HOME/upgrade.yay ]]; then echo -e "\n"; if cat $HOME/upgrade.yay | grep "$4" > /dev/null ; then rm $HOME/upgrade.yay; fi; fi
  if [[ -f $HOME/upgrade.paru ]]; then echo -e "\n"; if cat $HOME/upgrade.paru | grep "$4" > /dev/null ; then rm $HOME/upgrade.paru; fi; fi
