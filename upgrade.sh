...
pacdiffmeld ()
{
  ...
  echo -e "\n"; read -r -n 1 -p "Проверить файлы системных конфигураций на наличие резервных копий .conf? [y/N]: " pacsn;
  if [[ "$pacsn" = [yYlLдД] ]]; then 
...
# ----------------------------------------------------------------------
#Проверяются зависимости для yandex-browser...
...
if [[ "$updaur" = [yYlLдД] ]]; then
  ...
  if compgen -G "$HOME/upgrade.*" > /dev/null; 
    then postrun ;
      ...
      echo -e "\n"; read -r -n 1 -p "Признаков необходимости пост обработки нет. Провести ее принудительно? [y/N]: " post;
      if [[ "$post" = [yYlLдД] ]]; then postrun ; fi
  fi
fi
